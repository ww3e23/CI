...
export function AddDefectSheet({
  ...
  async function onPick(file: File | undefined, kind: 'plan' | 'photo') {
    ...
    reader.onload = () => {
      ...
      if (kind === 'plan') setPlanPhoto(url)
      else setPhotos((prev) => [...prev, url].slice(0, 6))
    }
    ...
    <>
      ...
      <div className="sheet" role="dialog" aria-label="新增缺失">
        ...
        <div className="field">
          ...
          {planPhoto && <img className="photo-thumb" src={planPhoto} alt="圖面位置" style={{ marginTop: 8 }} />}
        ...
        <button
          ...
          onClick={async () => {
            setSaving(true)
            setSyncMsg(cloudReady() ? '正在同步…' : '已儲存在本機（示範模式）')
            ...
            else if (d.syncState === 'failed') setSyncMsg('同步失敗，可稍後重試（資料已在本機）')
            else if (d.syncState === 'demo') setSyncMsg('已儲存在本機（示範模式）')
            else setSyncMsg('等待上傳')
            onClose()
          }}
        >
          儲存並同步雲端
